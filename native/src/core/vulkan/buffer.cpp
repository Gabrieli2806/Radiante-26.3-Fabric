...
void vk::DeviceLocalBuffer::uploadToStagingBuffer(void *src, size_t size, size_t offset) {
    // A transient staging buffer that has not been copied to the device yet is filled again (for example several
    // regions written before the next upload) instead of aborting the game.
    if (!persistStaging_ && stagingBuffer_ == VK_NULL_HANDLE) {
