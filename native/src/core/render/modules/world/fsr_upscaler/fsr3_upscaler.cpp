...
#ifdef DEBUG
    radiante::out() << "FSR3Upscaler::initialize called" << std::endl;
#endif

#ifndef MCVR_ENABLE_FFX_UPSCALER
    (void)config;
    radiante::err() << "FSR3Upscaler::initialize: FSR3 support not compiled in" << std::endl;
    return false;
#else
    if (m_initialized) {
#    ifdef DEBUG
        radiante::err() << "[DEBUG] FSR3: Destroying old context" << std::endl;
        std::flush(radiante::err());
#    endif
        destroy();
    }

#    ifdef DEBUG
    radiante::err() << "[DEBUG] FSR3: Assigning handles" << std::endl;
#    endif
    m_device = config.device;
    m_physicalDevice = config.physicalDevice;
#    ifdef DEBUG
    radiante::err() << "[DEBUG] FSR3: Device = " << (void *)m_device << std::endl;
    std::flush(radiante::err());
#    endif
...
#    ifdef DEBUG
    radiante::out() << "[FSR3] init config: render=" << m_renderWidth << "x" << m_renderHeight
...
#    ifdef DEBUG
    radiante::err() << "[DEBUG] FSR3: Calling createContext()" << std::endl;
    std::flush(radiante::err());
#    endif
    if (!createContext()) {
        radiante::err() << "FSR3Upscaler::initialize: Failed to create FFX context" << std::endl;
        return false;
    }
...
#    ifdef DEBUG
    radiante::err() << "=== FSR3 INITIALIZED ===" << std::endl;
    radiante::err() << "Render: " << m_renderWidth << "x" << m_renderHeight << " -> Display: " << m_displayWidth << "x"
              << m_displayHeight << std::endl;
    radiante::err() << "Context: " << (m_fsrContext != nullptr ? "OK" : "FAILED") << std::endl;
#    endif
...
#else
#    ifdef DEBUG
    radiante::err() << "[DEBUG] FSR3: Entering createContext" << std::endl;
    std::flush(radiante::err());
#    endif
...
#    ifdef DEBUG
            radiante::out() << "FSR3: Found " << versionCount << " available FSR version(s):" << std::endl;
            for (uint64_t i = 0; i < versionCount; ++i) {
                radiante::out() << "  Version[" << i << "] = 0x" << std::hex << fsrVersionIds[i] << std::dec;
                if (i < fsrVersionNames.size() && fsrVersionNames[i] != nullptr) {
                    radiante::out() << " (" << fsrVersionNames[i] << ")";
                }
...
#    endif
            ...
            radiante::err() << "FSR3: No alternative versions available" << std::endl;
        }
    }
...
#    ifdef DEBUG
    radiante::out() << "FSR3: Checking required extensions..." << std::endl;
#    endif
...
#    ifdef DEBUG
        radiante::err() << "  " << ext << ": " << (available ? "OK" : "MISSING") << std::endl;
#    endif
        if (!available && strcmp(ext, VK_KHR_PUSH_DESCRIPTOR_EXTENSION_NAME) == 0) {
            radiante::err() << "FSR3: Missing required extension: " << ext << std::endl;
        }
    }
    ...
    if (deviceProcAddr == nullptr) {
        radiante::err() << "FSR3 ERROR: customVkGetDeviceProcAddr is NULL!" << std::endl;
        return false;
    }
...
#    ifdef DEBUG
    radiante::err() << "FSR3: Function pointer check:" << std::endl;
    radiante::err() << "  vkCreateComputePipelines: " << (void *)testCreateComputePipelines << std::endl;
    radiante::err() << "  vkCmdDispatch: " << (void *)testCmdDispatch << std::endl;
    radiante::err() << "  vkCreateSampler: " << (void *)testCreateSampler << std::endl;
#    endif

    if (testCreateComputePipelines == nullptr || testCmdDispatch == nullptr) {
        radiante::err() << "FSR3 ERROR: Failed to get required Vulkan function pointers" << std::endl;
        return false;
    }

    backendDesc.vkDeviceProcAddr = deviceProcAddr;
#    ifdef DEBUG
    radiante::err() << "FSR3: vkDeviceProcAddr = " << reinterpret_cast<void *>(deviceProcAddr) << std::endl;
#    endif
    ...
    if (retCode == ffx::ReturnCode::Ok) {
        ...
        if (ffx::Query(reinterpret_cast<ffx::Context &>(m_fsrContext), providerQuery) == ffx::ReturnCode::Ok &&
            ...
            radiante::out() << "FSR3: Active provider = " << providerQuery.versionName << " (0x" << std::hex
                      << providerQuery.versionId << std::dec << ")" << std::endl;
        }
#    ifdef DEBUG
        radiante::out() << "FSR3: CreateContext succeeded!" << std::endl;
#    endif
        m_contextCreated = true;
        return true;
    }

    radiante::err() << "FSR3: CreateContext failed with error: " << toFFXReturnCodeString(retCode) << " ("
              << static_cast<uint32_t>(retCode) << ")" << std::endl;

    radiante::err() << "FSR3: Device=" << m_device << std::endl;
    radiante::err() << "FSR3: PhysicalDevice=" << m_physicalDevice << std::endl;

    if (retCode == ffx::ReturnCode::ErrorRuntimeError) {
        radiante::err() << "FSR3: Runtime error - check Vulkan validation layers for details" << std::endl;
    }
...
void FSR3Upscaler::destroyContext() {
    ...
    if (result != ffx::ReturnCode::Ok) {
        radiante::err() << "FSR3: DestroyContext failed with error: " << toFFXReturnCodeString(result) << " ("
...
#    ifdef DEBUG
        radiante::out() << "[FSR3] dispatch: render=" << input.renderWidth << "x" << input.renderHeight
...
#    endif
    ...
    if (retCode != ffx::ReturnCode::Ok) {
        radiante::err() << "FSR3 DISPATCH FAILED: " << toFFXReturnCodeString(retCode) << " ("
                  << static_cast<uint32_t>(retCode) << ")" << std::endl;
        return;
    }
    // else {
    ...
    //         radiante::err() << "FSR3 dispatch OK (count: " << dispatchCount << ")" << std::endl;
    //     }
    // }
